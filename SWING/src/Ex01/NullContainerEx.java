...
//		la.setSize(200, 20); // la를 200x20 크기로 지정
		...
		JButton c = new JButton("OK"); // 버튼 생성
		c.setLocation(200, 200); // 버튼의 위치 설정
		c.setSize(100, 100); // 버튼의 크기는 동일하게 50x20
		contentPane.add(c); // 버튼을 컨텐트팬에 부착

		// 9개의 버튼 컴포넌트를 생성하고 동일한 크기로 설정한다.
		// 위치는 서로 겹치게 설정한다.
		for(int i=1; i<=9; i++) {
			JButton b = new JButton(Integer.toString(i)); // 버튼 생성
			b.setLocation(i*20, i*20); // 버튼의 위치 설정
